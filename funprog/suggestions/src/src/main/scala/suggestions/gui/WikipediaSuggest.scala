...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map(button => suggestionList.selection.items.headOption.get)
    
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(selection => wikiPageResponseStream(selection))
