...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy[Char](x => x).toList.map(xs => (xs._2.head, xs._2.length))
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = { 
    def makeOLLAcc(freqs: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
      if (freqs.isEmpty) acc
      else makeOLLAcc(freqs.tail, Leaf(freqs.head._1,freqs.head._2) :: acc)
    }
    makeOLLAcc(freqs.sortBy(x=>x._2).reverse,List())
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(origTree: CodeTree, tree: CodeTree, bits: List[Bit], message: List[Char]): List[Char] = tree match {
      case l: Leaf => bits match {
        case head::tail => decodeAcc(origTree,origTree,head::tail,message ++ List(l.char))
        case _ => message ++ List(l.char)
      }
      case f: Fork => bits match {
        case 0::tail => decodeAcc(origTree,f.left,tail,message) 
        case 1::tail => decodeAcc(origTree,f.right,tail,message)
        case _ => message
      }
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case (c,l) :: rest => if(char == c) l else codeBits(rest)(char)
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def addParent(b:Bit)(t: (Char, List[Bit])): (Char, List[Bit]) = t match{
      case (c,l) => (c,b::l)
    }
    a.map(addParent(0)) ::: b.map(addParent(1))
  }
