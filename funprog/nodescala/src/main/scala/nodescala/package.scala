...
package object nodescala {
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = Await.result(f, Duration("-1"))
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete {
        // Call cont after f completes successfully
        case Success(v) => try cont(f) catch { case NonFatal(t) => p failure t }
        // Fail gracefully if future fails
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      ...
      f.onComplete {
        // Call cont after f completes successfully
        case Success(v) => f.value match { 
	        case Some(x) => p success cont(x)
	        case None => p complete f.asInstanceOf[Failure[S]]
        }
        // Fail gracefully if future fails
        case f: Failure[_] => p complete f.asInstanceOf[Failure[S]]
      }
